...
def tanhRNNrsc(n_input, n_hidden, n_output, rsc=11,
               input_type='real', out_every_t=False, loss_function='CE'):
    """Tanh RNN with RSC"""
    np.random.seed(1234)
    rng = np.random.RandomState(1234)

    x, y = initialize_data_nodes(loss_function, input_type, out_every_t)
    # x.tag.test_value = np.random.randint(1, 7, (220, 30))
    # y.tag.test_value = np.random.randint(1, 7, (220, 30))
    inputs = [x, y]
    ...
    S = theano.shared(
        np.zeros((n_hidden, n_hidden)).astype(theano.config.floatX))
    ...
    parameters = [V, W, S, out_mat, hidden_bias, out_bias]

    def recurrence(x_t, y_t, h_prev, h_tms, cost_prev, acc_prev,
                   V, W, S, out_mat, hidden_bias, out_bias):
        if loss_function == 'CE':
            data_lin_output = V[x_t]
        else:
            data_lin_output = T.dot(T.cast(x_t, theano.config.floatX), V)

        h_t = T.tanh(T.dot(h_prev, W) + \
                     data_lin_output + \
                     T.dot(h_tms, S) + \
    ...
    non_sequences = [V, W, S, out_mat, hidden_bias, out_bias]
    if out_every_t:
        sequences = [x, y]
    else:
        sequences = [
    ...
    outputs_info = [
        dict(initial=T.tile(theano.shared(np.zeros((rsc, 1, n_hidden),
                                                   dtype=theano.config.floatX)),
                            [1, x.shape[1], 1]),
             taps=[-1, -rsc]),
    ...
    [hidden_states, cost_steps, acc_steps], updates = theano.scan(
        fn=recurrence,
        sequences=sequences,
    ...
    if not out_every_t:
        lin_output = T.dot(hidden_states[-1,:,:], out_mat) + \
                     out_bias.dimshuffle('x', 0)
...
def complex_RNN(n_input, n_hidden, n_output, input_type='real',
                out_every_t=False, loss_function='CE'):
    ...
    index_permute_long = np.concatenate((index_permute,
                                         index_permute + n_hidden))
    swap_re_im = np.concatenate((np.arange(n_hidden, 2*n_hidden),
                                 np.arange(n_hidden)))
    
    # define the recurrence used by theano.scan
    def recurrence(x_t, y_t, h_prev, cost_prev, acc_prev,
                   theta, V, hidden_bias, out_bias, U):
    ...
    if not out_every_t:
        lin_output = T.dot(hidden_states[-1,:,:], U) + \
                     out_bias.dimshuffle('x', 0)
        costs = compute_cost_t(lin_output, loss_function, y)
    else:
        cost = cost_steps.mean()
